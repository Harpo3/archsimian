...
Replay intervals – you can adjust the 'global' time between repeats with different intervals for each track rating. By using ratios of repeat frequency with average listening time, the program can determine whether a given track should be added to the playlist. Variety takes precedence over repeat frequency. For example, if your setting for two-star tracks is to repeat every three years, but based on your actual listening history, they were heard within the last two years, they still might be replayed before three years because selection is based on the 'relative frequency' of two-star tracks to the other track rating categories. You might need to set a lower repeat frequency than what you desire (need three years, set to four years) to transition to lower repeat frequencies.
